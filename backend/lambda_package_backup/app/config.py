...
class Settings(BaseSettings):
    ...
    supabase_service_role_key: Optional[str] = None  # For bypassing RLS in Lambda
...
def get_supabase_client():
    ...
    Uses service role key if available (for Lambda/backend operations that need RLS bypass),
    ...
    if _supabase_client is None:
        ...
        if not settings.supabase_url:
            raise ValueError("SUPABASE_URL is required for Supabase client")
        
        # Prefer service role key for backend operations (bypasses RLS)
        api_key = settings.supabase_service_role_key or settings.supabase_anon_key
        
        if not api_key:
            raise ValueError("SUPABASE_SERVICE_ROLE_KEY or SUPABASE_ANON_KEY is required for Supabase client")
