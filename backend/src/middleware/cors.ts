 ...
 * @returns Hono middleware handler
 */
export function devCorsMiddleware(): MiddlewareHandler {
 ...
 * @returns Hono middleware handler
 */
export function strictCorsMiddleware(allowedOrigins: string[]): MiddlewareHandler {
 ...
 */
export function setCorsHeaders(c: Context, origin?: string): void {
  const settings = getSettings();
 ...
 */
export function handlePreflight(c: Context): Response {
  setCorsHeaders(c);
