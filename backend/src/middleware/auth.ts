 ...
 */
const STAGE_PREFIXES: string[] = ['/development', '/production', '/staging'];
 ...
 */
async function fetchJWKS(url: string): Promise<jose.JWK[]> {
...
async function getPublicKeyFromJWKS(
  ...
  const importedKey = await jose.importJWK(key, key.alg as string);
 ...
 */
async function verifySupabaseToken(token: string, settings: Settings): Promise<JWTPayload> {
 ...
 */
async function verifyCognitoToken(token: string, settings: Settings): Promise<JWTPayload> {
  ...
  // Build JWKS URL
  const jwksUrl =
    `https://cognito-idp.${settings.cognitoRegion}.amazonaws.com/` +
