...
import type { StickyDb } from '../schemas/dashboard.js';
 ...
 */
export class StickyRepository extends BaseRepository<StickyDb> {
  /**
   ...
   */
  async getByOwner(ownerType: string, ownerId: string): Promise<StickyDb[]> {
   ...
   */
  async getIncomplete(ownerType: string, ownerId: string): Promise<StickyDb[]> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select('*')
      .eq('owner_type', ownerType)
   ...
   */
  async getCompleted(ownerType: string, ownerId: string, limit = 10): Promise<StickyDb[]> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select('*')
      .eq('owner_type', ownerType)
   ...
   */
  async countByStatus(ownerType: string, ownerId: string, completed: boolean): Promise<number> {
   ...
   */
  async markCompleted(id: string): Promise<StickyDb | null> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .update({
        completed: true,
   ...
   * @returns The updated sticky if found, null otherwise.
   */
  async markIncomplete(id: string): Promise<StickyDb | null> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .update({
        completed: false,
