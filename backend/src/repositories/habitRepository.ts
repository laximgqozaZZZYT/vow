...
export class HabitRepository extends BaseRepository<Habit> {
   ...
   */
  async getActiveDoHabits(ownerType: string, ownerId: string): Promise<Habit[]> {
   ...
   */
  async findByName(ownerType: string, ownerId: string, name: string): Promise<Habit | null> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select('*')
      .eq('owner_type', ownerType)
   ...
   */
  async getByOwner(ownerType: string, ownerId: string, activeOnly = false): Promise<Habit[]> {
   ...
   */
  async getHabitsByGoal(goalId: string, activeOnly = true): Promise<Habit[]> {
    let query = this.supabase
      .from(this.tableName)
      .select('*')
      .eq('goal_id', goalId);
   ...
   */
  async getHabitsWithTriggers(): Promise<Habit[]> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select('*')
