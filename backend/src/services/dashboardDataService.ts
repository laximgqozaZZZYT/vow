...
/**
 * Extended Habit type with additional fields from database.
 */
interface ExtendedHabit extends Habit {
  type?: string;
  must?: number;
  workload_total?: number;
  workload_total_end?: number;
  time?: string;
  end_time?: string;
  due_date?: string;
  repeat?: string;
...
export class DashboardDataService {
   ...
   */
  private isHabitCumulativelyCompleted(habit: ExtendedHabit, activities: Activity[]): boolean {
    ...
    const habitActivities = activities.filter(
      (a) => a.habit_id === habit.id && a.kind === 'complete'
    );
   ...
   */
  async getDailyProgress(ownerId: string, ownerType = 'user'): Promise<DailyProgressData> {
    try {
      // Get active habits with type="do"
      const habits = (await this.habitRepo.getActiveDoHabits(
        ownerType,
        ownerId
      )) as ExtendedHabit[];
      ...
      const allActivities = await this.activityRepo.getActivitiesByOwnerInRange(
        ownerType,
        ownerId,
        new Date(0),
        new Date()
      ...
      // Build progress list
      const progressList: DailyProgressItem[] = [];
      ...
      throw new DataFetchError(
        `Failed to get daily progress: ${error instanceof Error ? error.message : String(error)}`,
   ...
   */
  async getStatistics(ownerId: string, ownerType = 'user'): Promise<StatisticsData> {
    try {
      ...
      const sortedByProgress = [...dailyProgress.habits].sort(
        (a, b) => b.progressRate - a.progressRate
      );
      const top3Habits: Top3Habit[] = sortedByProgress.slice(0, 3).map((h) => ({
        habitId: h.habitId,
   ...
   */
  async getNextHabits(ownerId: string, ownerType = 'user'): Promise<NextHabitsData> {
    try {
      // Get all habits for the owner
      const habits = (await this.habitRepo.getByOwner(ownerType, ownerId, true)) as ExtendedHabit[];
      ...
      const candidates: Array<{ habit: ExtendedHabit; startTime: Date }> = [];

      for (const habit of habits) {
        // Skip completed habits
        if ((habit as unknown as { completed?: boolean }).completed) {
          continue;
        }
      ...
      // Build response
      const habitItems: NextHabitItem[] = limited.map(({ habit, startTime }) => {
        const todayStr = now.toISOString().slice(0, 10);
      ...
      throw new DataFetchError(
        `Failed to get next habits: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error : undefined
      );
    }
  }
  ...
  async getStickies(ownerId: string, ownerType = 'user'): Promise<StickiesData> {
    try {
      ...
      throw new DataFetchError(
        `Failed to get stickies: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error : undefined
      );
    }
  }
