...
import type { SlackRepository } from '../repositories/slackRepository.js';
...
import type { SlackIntegrationService } from './slackService.js';
import { getSlackService } from './slackService.js';
import { SlackBlockBuilder, type WeeklyReportData } from './slackBlockBuilder.js';
import { decryptToken } from '../utils/encryption.js';
import { DataFetchError } from '../errors/index.js';
import { getLogger } from '../utils/logger.js';
import type { SlackMessage } from '../schemas/slack.js';
...
export class WeeklyReportGenerator {
  ...
  async generateReport(
    ...
    try {
      ...
      let bestStreak = 0;
      let bestStreakHabit = '';
      const habitsNeedingAttention: string[] = [];
   ...
   */
  async sendWeeklyReport(ownerId: string, ownerType = 'user'): Promise<boolean> {
    try {
      ...
      const encryptedToken = connection['access_token'] as string;
      ...
      logger.error(
        `Error sending weekly report for ${ownerId}`,
   ...
   */
  async sendAllWeeklyReports(supabaseClient?: SupabaseClient): Promise<number> {
    let sentCount = 0;

    // Get all valid Slack connections with report preferences using repository
    const connections = await this.slackRepo.getValidConnectionsForReports(
      0, // Filtering done in service layer
      '00:00' // Filtering done in service layer
    );

    for (const connection of connections) {
      const ownerType = (connection['owner_type'] as string) ?? 'user';
      ...
      try {
        ...
        logger.error(
          `Error processing weekly report for ${ownerId}`,
          error instanceof Error ? error : new Error(String(error))
        );
        continue;
      }
   ...
   */
  private async getActivitiesForWeek(
    ownerType: string,
    ownerId: string,
    weekStart: Date,
    weekEnd: Date
