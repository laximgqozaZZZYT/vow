...
async function saveOAuthState(
  ...
  try {
    ...
    logger.error('Failed to save OAuth state', e as Error);
    return false;
  }
}
...
async function getAndDeleteOAuthState(
  ...
  try {
    ...
    return {
      owner_type: data['owner_type'] as string,
      owner_id: data['owner_id'] as string,
      redirect_uri: (data['redirect_uri'] as string) ?? '',
      timestamp: new Date(data['created_at'] as string).getTime() / 1000,
    };
  } catch (e) {
    logger.error('Failed to get OAuth state', e as Error);
    return null;
  }
}
 ...
 */
async function cleanupExpiredOAuthStates(supabase: SupabaseClient): Promise<void> {
  try {
    const now = new Date().toISOString();
    await supabase.from('slack_oauth_states').delete().lt('expires_at', now);
  } catch (e) {
    logger.warning('Failed to cleanup expired OAuth states', { error: (e as Error).message });
  }
}
...
// Cache for JWKS
let supabaseJwksCache: { keys: jose.JWK[]; fetchedAt: number } | null = null;
const JWKS_CACHE_TTL = 3600 * 1000; // 1 hour in milliseconds

/**
 * Fetch and cache Supabase JWKS.
 */
async function getSupabaseJwks(settings: Settings): Promise<jose.JWK[]> {
  const now = Date.now();
...
async function verifyWithJwks(
  ...
  const { payload } = await jose.jwtVerify(token, publicKey, {
    algorithms: [alg as jose.JWTVerifyOptions['algorithms'] extends (infer T)[] ? T : never],
...
async function verifyJwtToken(
  ...
  try {
    ...
    logger.error('JWT verification failed', e as Error);
 ...
 */
function getCurrentUser(c: Context): { id: string; type: string } {
 ...
 */
export function createSlackOAuthRouter(): Hono {
  ...
  router.get('/connect', async (c) => {
    ...
    try {
      ...
      return c.json(
        { error: 'Not authenticated. Please provide token.' },
        401
      );
    }
  ...
  router.get('/callback', async (c) => {
    ...
    try {
      ...
      logger.error('Token exchange failed', e as Error);
      return c.redirect(
        getErrorRedirect('token_exchange_failed', (e as Error).message, frontendBase)
      );
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return c.redirect(
        getErrorRedirect('token_exchange_failed', tokenResponse.error ?? 'Unknown error', frontendBase)
      );
    }
    ...
    try {
      ...
      logger.error('Failed to save connection', e as Error);
      return c.redirect(
        getErrorRedirect('connection_failed', (e as Error).message, frontendBase)
      );
    }
  ...
  router.get('/status', async (c) => {
    ...
    const response: SlackConnectionStatus = {
  ...
  router.get('/preferences', async (c) => {
    ...
    if (!preferences) {
      const defaultPreferences: SlackPreferencesResponse = {
  ...
  router.put('/preferences', async (c) => {
    ...
    const preferences: SlackPreferencesUpdate = parseResult.data;
