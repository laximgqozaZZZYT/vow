# セルフケア・美容 (Self-Care & Beauty)

カテゴリID: self-care-beauty

## Habits

### スキンケア

#### 朝のスキンケア
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 07:00
- duration: 10
- reason: 朝のスキンケアは肌を保護し、一日中健康な肌を維持します。
- difficultyLevel: beginner
- habitStackingTriggers: 洗顔後, 起床後

#### 夜のスキンケア
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: 15
- reason: 夜のスキンケアは肌の回復を助け、エイジングケアに効果的です。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴後, 就寝前

#### 日焼け止めを塗る
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 08:00
- duration: null
- reason: 紫外線対策は肌の老化防止と皮膚がん予防に最も重要です。
- difficultyLevel: beginner
- habitStackingTriggers: 朝のスキンケア後, 外出前

#### 保湿をしっかりする
- type: do
- frequency: daily
- suggestedTargetCount: 2
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: 保湿は肌のバリア機能を維持し、乾燥やシワを防ぎます。
- difficultyLevel: beginner
- habitStackingTriggers: 洗顔後, 入浴後

#### パックをする
- type: do
- frequency: weekly
- suggestedTargetCount: 2
- workloadUnit: 回
- triggerTime: null
- duration: 15
- reason: 週に数回のパックは、肌に集中的な栄養を与えます。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴後, 夜のスキンケア時

#### 角質ケアをする
- type: do
- frequency: weekly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 10
- reason: 定期的な角質ケアは、肌のターンオーバーを促進します。
- difficultyLevel: intermediate
- habitStackingTriggers: 入浴時, 週末

#### 美容液を使う
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: null
- reason: 美容液は有効成分を肌に届け、特定の肌悩みに効果的です。
- difficultyLevel: intermediate
- habitStackingTriggers: 化粧水後, 夜のスキンケア時

#### アイクリームを塗る
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: null
- reason: 目元は皮膚が薄く、専用のケアでシワやクマを予防できます。
- difficultyLevel: intermediate
- habitStackingTriggers: 美容液後, 夜のスキンケア時

### ヘアケア

#### 髪をブラッシングする
- type: do
- frequency: daily
- suggestedTargetCount: 2
- workloadUnit: 回
- triggerTime: null
- duration: 3
- reason: ブラッシングは頭皮の血行を促進し、髪の艶を出します。
- difficultyLevel: beginner
- habitStackingTriggers: 起床後, 就寝前

#### トリートメントをする
- type: do
- frequency: weekly
- suggestedTargetCount: 2
- workloadUnit: 回
- triggerTime: null
- duration: 10
- reason: トリートメントは髪のダメージを補修し、潤いを与えます。
- difficultyLevel: beginner
- habitStackingTriggers: シャンプー後

#### 頭皮マッサージをする
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 5
- reason: 頭皮マッサージは血行を促進し、健康な髪の成長を助けます。
- difficultyLevel: beginner
- habitStackingTriggers: シャンプー時, 入浴時

#### ヘアオイルを使う
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: ヘアオイルは髪を保護し、艶やかな髪を維持します。
- difficultyLevel: beginner
- habitStackingTriggers: ドライヤー前, 朝のスタイリング時

#### 定期的にカットする
- type: do
- frequency: monthly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 60
- reason: 定期的なカットは、髪の健康と清潔感を維持します。
- difficultyLevel: beginner
- habitStackingTriggers: 月に一度

### ボディケア

#### ボディクリームを塗る
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: 5
- reason: ボディクリームは全身の肌を保湿し、乾燥を防ぎます。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴後

#### ボディスクラブをする
- type: do
- frequency: weekly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 10
- reason: ボディスクラブは古い角質を除去し、肌を滑らかにします。
- difficultyLevel: intermediate
- habitStackingTriggers: 入浴時, 週末

#### ハンドクリームを塗る
- type: do
- frequency: daily
- suggestedTargetCount: 3
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: 手は年齢が出やすい部分です。こまめな保湿が大切です。
- difficultyLevel: beginner
- habitStackingTriggers: 手洗い後, 仕事の合間

#### リップケアをする
- type: do
- frequency: daily
- suggestedTargetCount: 3
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: 唇は乾燥しやすいため、こまめなケアで潤いを保ちましょう。
- difficultyLevel: beginner
- habitStackingTriggers: 朝, 昼, 夜

#### ネイルケアをする
- type: do
- frequency: weekly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 20
- reason: 清潔で整った爪は、清潔感と美しさを演出します。
- difficultyLevel: beginner
- habitStackingTriggers: 週末, 入浴後

#### フットケアをする
- type: do
- frequency: weekly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 15
- reason: 足のケアは、健康と美しさの両方に重要です。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴後, 週末

### オーラルケア

#### 歯を磨く（朝晩）
- type: do
- frequency: daily
- suggestedTargetCount: 2
- workloadUnit: 回
- triggerTime: null
- duration: 3
- reason: 適切な歯磨きは虫歯や歯周病を予防し、口臭も防ぎます。
- difficultyLevel: beginner
- habitStackingTriggers: 起床後, 就寝前

#### フロスを使う
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: 2
- reason: フロスは歯ブラシでは届かない歯間の汚れを除去します。
- difficultyLevel: beginner
- habitStackingTriggers: 歯磨き後

#### マウスウォッシュを使う
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: マウスウォッシュは口内を清潔に保ち、口臭を予防します。
- difficultyLevel: beginner
- habitStackingTriggers: 歯磨き後, フロス後

#### 舌を磨く
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: 舌の汚れは口臭の原因になります。毎日のケアが大切です。
- difficultyLevel: beginner
- habitStackingTriggers: 歯磨き時

#### 定期的に歯科検診を受ける
- type: do
- frequency: monthly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 60
- reason: 定期検診は、問題の早期発見と予防に重要です。
- difficultyLevel: beginner
- habitStackingTriggers: 3-6ヶ月に一度

### リラクゼーション

#### 入浴剤を使う
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 21:00
- duration: null
- reason: 入浴剤はリラックス効果を高め、肌にも良い影響を与えます。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴時

#### アロマを楽しむ
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: アロマの香りはリラックス効果があり、気分を整えてくれます。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴時, 就寝前, 仕事中

#### セルフマッサージをする
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: 10
- reason: セルフマッサージは血行を促進し、むくみを解消します。
- difficultyLevel: beginner
- habitStackingTriggers: 入浴後, 就寝前

#### 半身浴をする
- type: do
- frequency: weekly
- suggestedTargetCount: 2
- workloadUnit: 回
- triggerTime: null
- duration: 20
- reason: 半身浴は体を芯から温め、デトックス効果があります。
- difficultyLevel: intermediate
- habitStackingTriggers: 週末, 休日

#### サウナに行く
- type: do
- frequency: weekly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 90
- reason: サウナは血行促進とリラックス効果で、心身をリフレッシュします。
- difficultyLevel: intermediate
- habitStackingTriggers: 週末, 休日

### 身だしなみ

#### 服を選ぶ時間を作る
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 22:00
- duration: 5
- reason: 前日に服を選ぶことで、朝の時間に余裕ができます。
- difficultyLevel: beginner
- habitStackingTriggers: 就寝前

#### 靴を磨く
- type: do
- frequency: weekly
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: null
- duration: 10
- reason: 清潔な靴は、全体の印象を大きく左右します。
- difficultyLevel: beginner
- habitStackingTriggers: 週末, 外出前

#### 姿勢を意識する
- type: do
- frequency: daily
- suggestedTargetCount: 3
- workloadUnit: 回
- triggerTime: null
- duration: null
- reason: 良い姿勢は、見た目の印象と健康の両方に影響します。
- difficultyLevel: beginner
- habitStackingTriggers: デスクワーク中, 歩行中

#### 笑顔の練習をする
- type: do
- frequency: daily
- suggestedTargetCount: 1
- workloadUnit: 回
- triggerTime: 07:00
- duration: 1
- reason: 自然な笑顔は、最高の美容法であり、人間関係も良くします。
- difficultyLevel: beginner
- habitStackingTriggers: 鏡を見た時, 起床後

## Goals

### 美肌づくり
- icon: ✨
- description: 毎日のスキンケアで、健康で美しい肌を手に入れる
- reason: 美しい肌は自信につながり、第一印象を良くします。
- suggestedHabits:
  - 朝のスキンケア
  - 夜のスキンケア
  - 日焼け止めを塗る
  - 保湿をしっかりする

### 美髪づくり
- icon: 💇
- description: 髪のケアを習慣化し、艶やかで健康な髪を維持する
- reason: 美しい髪は、全体の印象を大きく左右します。
- suggestedHabits:
  - 髪をブラッシングする
  - トリートメントをする
  - 頭皮マッサージをする
  - ヘアオイルを使う

### 口元の美しさ
- icon: 😁
- description: オーラルケアを徹底し、健康で美しい口元を保つ
- reason: 清潔な口元は、健康と美しさの両方に重要です。
- suggestedHabits:
  - 歯を磨く（朝晩）
  - フロスを使う
  - 舌を磨く
  - 定期的に歯科検診を受ける

### 全身のケア
- icon: 🧴
- description: 全身のケアを習慣化し、健康で美しい体を維持する
- reason: 全身のケアは、自分を大切にする行為であり、自己肯定感を高めます。
- suggestedHabits:
  - ボディクリームを塗る
  - ハンドクリームを塗る
  - ネイルケアをする
  - フットケアをする

### リラックス習慣
- icon: 🛁
- description: リラクゼーションを日常に取り入れ、心身をリフレッシュする
- reason: リラックスする時間は、美容と健康の両方に不可欠です。
- suggestedHabits:
  - 入浴剤を使う
  - アロマを楽しむ
  - セルフマッサージをする
  - 半身浴をする

### 清潔感のある身だしなみ
- icon: 👔
- description: 身だしなみを整え、清潔感のある印象を与える
- reason: 清潔感は、社会生活において最も基本的で重要な要素です。
- suggestedHabits:
  - 服を選ぶ時間を作る
  - 靴を磨く
  - 姿勢を意識する
  - 笑顔の練習をする
