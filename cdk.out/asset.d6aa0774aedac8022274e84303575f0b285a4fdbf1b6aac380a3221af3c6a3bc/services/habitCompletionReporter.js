...
export class HabitCompletionReporter {
     ...
     */
    async completeHabitByName(ownerId, habitName, source = 'slack', ownerType = 'user') {
        try {
            // Find habit by name (case-insensitive)
            const habits = await this.habitRepo.getByOwner(ownerType, ownerId);
            // Try exact match first
            let habit = habits.find((h) => h.name.toLowerCase() === habitName.toLowerCase());
            // Try partial match if no exact match
            if (!habit) {
                habit = habits.find((h) => h.name.toLowerCase().includes(habitName.toLowerCase()));
            }
            if (!habit) {
            ...
            throw new DataFetchError(`Failed to complete habit by name: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     * @returns Tuple of [success, message, habitData].
     */
    async completeHabitById(ownerId, habitId, _source = 'slack', ownerType = 'user') {
        try {
            ...
            throw new DataFetchError(`Failed to complete habit: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     */
    async getIncompleteHabitsToday(ownerId, ownerType = 'user') {
        try {
            const habits = await this.habitRepo.getByOwner(ownerType, ownerId);
            const [start, end] = this.getJstDayBoundaries();
            const incomplete = [];
            for (const habit of habits) {
                ...
                const isCompleted = await this.activityRepo.hasCompletionToday(habit.id, start, end);
                if (!isCompleted) {
            ...
            throw new DataFetchError(`Failed to get incomplete habits: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     */
    async getAllHabitsWithStatus(ownerId, ownerType = 'user') {
        try {
            const habits = await this.habitRepo.getByOwner(ownerType, ownerId);
            const [start, end] = this.getJstDayBoundaries();
            const result = [];
            ...
            throw new DataFetchError(`Failed to get habits with status: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     */
    async getHabitStreak(habitId, _ownerType = 'user', _ownerId) {
        return withRetry(async () => {
            try {
                ...
                throw new DataFetchError(`Failed to calculate streak: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
            }
        });
    }
    /**
     ...
     */
    async getTodaySummary(ownerId, ownerType = 'user') {
        try {
            ...
            throw new DataFetchError(`Failed to get today summary: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     */
    async incrementHabitProgress(ownerId, habitId, amount, _source = 'slack', ownerType = 'user') {
        try {
            ...
            throw new DataFetchError(`Failed to increment habit progress: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    }
    // ========================================================================
     ...
     */
    findSimilarHabits(name, habits, threshold = 0.6) {
        const similar = [];
...
//# sourceMappingURL=habitCompletionReporter.js.map