...
import { SlackAPIError, getUserFriendlyMessage } from '../errors';
 ...
 */
async function sendNotConnectedResponse(slackService, responseUrl) {
    const blocks = SlackBlockBuilder.notConnected();
    await slackService.sendResponse(responseUrl, 'VOWアカウントとの接続が見つかりません。', blocks, true);
}
// =============================================================================
// Action Handlers
// =============================================================================
 ...
 * - 6.5: Handle already completed habits
 */
async function handleHabitDone(completionReporter, slackService, responseUrl, ownerType, ownerId, habitId) {
    const [success, _message, data] = await completionReporter.completeHabitById(ownerId, habitId, 'slack', ownerType);
    let blocks;
    let responseText;
    if (success && data) {
 ...
 */
async function handleHabitSkip(slackRepo, habitRepo, slackService, responseUrl, ownerType, ownerId, habitId) {
 ...
 */
async function handleHabitLater(slackRepo, habitRepo, slackService, responseUrl, ownerType, ownerId, habitId) {
 ...
 */
async function handleHabitIncrement(completionReporter, slackService, responseUrl, ownerType, ownerId, habitId) {
    try {
        // Call increment_habit_progress
        const [success, message, resultData] = await completionReporter.incrementHabitProgress(ownerId, habitId, undefined, // Use default workload_per_count
        'slack', ownerType);
        if (!success) {
            ...
            const errorBlocks = SlackBlockBuilder.dashboardError(message || 'この習慣は見つかりませんでした。');
            await slackService.sendResponse(responseUrl, '習慣が見つかりません。', errorBlocks, false);
            return;
        }
        ...
        const errorBlocks = SlackBlockBuilder.dashboardError('進捗の更新中にエラーが発生しました。再度お試しください。');
        await slackService.sendResponse(responseUrl, 'エラーが発生しました。', errorBlocks, false);
    }
}
/**
 ...
 */
async function processBlockAction(payload, settings) {
    ...
    try {
        ...
        if (actionId.startsWith('habit_done_')) {
            await handleHabitDone(completionReporter, slackService, responseUrl, ownerType, ownerId, habitId);
        }
        else if (actionId.startsWith('habit_skip_')) {
            await handleHabitSkip(slackRepo, habitRepo, slackService, responseUrl, ownerType, ownerId, habitId);
        }
        else if (actionId.startsWith('habit_later_')) {
            await handleHabitLater(slackRepo, habitRepo, slackService, responseUrl, ownerType, ownerId, habitId);
        }
        else if (actionId.startsWith('habit_increment_')) {
            await handleHabitIncrement(completionReporter, slackService, responseUrl, ownerType, ownerId, habitId);
        }
        else {
        ...
        if (error instanceof SlackAPIError) {
            logger.error('Slack API error processing block action', error, {
                action_id: actionId,
                habit_id: habitId,
                owner_id: ownerId,
            });
        ...
        try {
            ...
            await slackService.sendResponse(responseUrl, errorMessage, SlackBlockBuilder.errorMessage(errorMessage), true);
        }
        catch {
 ...
 */
export function createSlackInteractionsRouter() {
...
//# sourceMappingURL=slackInteractions.js.map