/**
 * AI Coach System Prompt for MCP Chat
 *
 * This prompt instructs the AI to respond in JSON format (AICandidateResponse).
 * Used when communicating with MCP server for AI coaching functionality.
 *
 * @module constants/ai-coach-prompt
 ...
 * AIコーチシステムプロンプト (日本語)
 *
 * JSON形式（AICandidateResponse）での応答を指示するプロンプト
 */
export const AI_COACH_SYSTEM_PROMPT_JA = `あなたはVOW（習慣・目標トラッカー）のAIコーチです。
 ...
 * AIコーチシステムプロンプト (英語)
 */
export const AI_COACH_SYSTEM_PROMPT_EN = `You are an AI coach for VOW (habit and goal tracker).
 ...
 * ロケールに応じたプロンプトを取得
 */
export function getAICoachSystemPrompt(locale: 'ja' | 'en' = 'ja'): string {
  return locale === 'en' ? AI_COACH_SYSTEM_PROMPT_EN : AI_COACH_SYSTEM_PROMPT_JA;
}
